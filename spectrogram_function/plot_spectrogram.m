...
%
% @usage        [handle] = plot_spectrogram(speech, fs, T, w, nfft, Slim, alpha, cmap, cbar, type);
%
% @examples     [handle] = plot_spectrogram(speech, 8000, [18 1], @hamming, 1024, [-45 -2], false, 'default', false, 'lp');
%               [handle] = plot_spectrogram(speech, 8000, [18 1], @hamming, 1024, [-45 -2], [1 -0.97], 'default', true, 'per');
%______________________________________________________________________________________________________________________
function [handle] = plot_spectrogram(s, fs, T, w, nfft, Slim, alpha, cmap, cbar, type)
...
%    S(S<eps) = eps;                     % prevent zero spectral magnitude values 
    ...
    handle = imagesc(T, F, S);
    %handle = imagesc(T, F, S, 'CDataMapping', 'direct');
    axis('xy'); 
    axis([0 N/fs  0 fs/2]);
    xlabel('Time (s)', 'FontSize', 18, 'FontWeight', 'n');
    ylabel('Freq. (Hz)', 'FontSize', 18, 'FontWeight', 'n');
